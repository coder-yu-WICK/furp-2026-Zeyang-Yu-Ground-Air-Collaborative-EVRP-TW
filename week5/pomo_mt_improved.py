...
# Week 5 must come BEFORE week4 in path so that `from config import`
# resolves to week5/config.py, not week4/config.py.
if _W5 in sys.path:
    sys.path.remove(_W5)
...
if _W4 in sys.path:
    sys.path.remove(_W4)
sys.path.insert(1, _W4)  # after week5
...
# Import week4 POMO model classes directly (same package name 'algorithms')
from algorithms.pomo.pomo_model import POMOModel
from algorithms.pomo.pomo_env import POMOEnv
from algorithms.pomo.pomo_problem import augment_xy_by_8_fold

# Week 5 improvements
from tw_aware_clustering import cluster_customers_tw_aware
from drone_post_processing import apply_drone_postprocessing
from adaptive_clustering import cluster_with_params
from drone_reopt import apply_drone_postprocessing_with_reopt

# Week 6 cluster feasibility (Task A)
import importlib.util as _importlib_util
_cf_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'week6', 'cluster_feasibility.py')
_cf_spec = _importlib_util.spec_from_file_location("cluster_feasibility", _cf_path)
_cluster_feasibility = _importlib_util.module_from_spec(_cf_spec)
_cf_spec.loader.exec_module(_cluster_feasibility)
ensure_temporal_feasibility = _cluster_feasibility.ensure_temporal_feasibility
...
class ImprovedPOMOSolver:
    ...
    Original 4 variants:
      - baseline:          spatial-only clustering, no drones
    ...
    Extended 6 variants (Week 5.5):
      - adaptive_tw:       Adaptive TW-aware clustering, no drones
      - adaptive_tw_drone: Adaptive TW-aware + drone + re-opt
      ...
      - hybrid_drone:      Hybrid + drone post-processing + re-opt
    """
    ...
    def solve_cluster(self, cluster_instance, seed=42):
        ...
        from algorithms.pomo.pomo_problem import instance_to_pomo_features
        ...
        for route in all_routes:
            ...
            solutions.append(TruckDroneSolution(truck_routes, [], cluster_instance))

        return solutions

    def _get_cluster_strategy(self, variant):
        """Map variant name to clustering strategy."""
        if variant in ('baseline', 'drone_only'):
            return 'spatial', None
        elif variant in ('tw_aware', 'tw_aware_drone'):
            return 'tw_aware', 0.5
        elif variant in ('adaptive_tw', 'adaptive_tw_drone'):
            return 'adaptive_tw', 0.4
        elif variant in ('angle', 'angle_drone'):
            return 'angle', None
        elif variant in ('hybrid', 'hybrid_drone', 'hybrid_drone_no_reopt'):
            return 'hybrid', 0.4
        else:
            return 'spatial', None

    def _uses_drones(self, variant):
        """Check if variant uses drone post-processing."""
        return variant in ('drone_only', 'tw_aware_drone',
                          'adaptive_tw_drone', 'angle_drone', 'hybrid_drone',
                          'hybrid_drone_no_reopt')

    def _uses_reopt(self, variant):
        """Check if variant uses drone + re-optimization."""
        return variant in ('adaptive_tw_drone', 'angle_drone', 'hybrid_drone')

    def solve(self, instance, n_trucks, variant='baseline', n_runs=1,
              seed=42, tw_beta=0.5, drone_endurance='medium',
              check_tw_feasibility=True):
        """
        Solve full instance.

        Supports all 10 variants (4 original + 6 extended).

        Drone strategy:
          - drone_only, tw_aware_drone: basic cross-route drone insertion
          - adaptive_tw_drone, angle_drone, hybrid_drone:
            cross-route drone insertion + POMO re-optimization of affected routes
        ...
        """
        use_drones = self._uses_drones(variant)
        use_reopt = self._uses_reopt(variant)
        ...
        for run in range(n_runs):
            ...
            if strategy == 'spatial':
                from week4_pomo_mt import cluster_customers as spatial_cluster
            ...
            # ── Step 1.5: Temporal feasibility check (Task A) ──
            ...
            # ── Step 4: Drone post-processing ──
            if use_drones:
                if use_reopt:
                    # Enhanced: drone insertion + POMO re-optimization
                    sol, cost_saved, n_drones, reopt_improvement = \
                        apply_drone_postprocessing_with_reopt(
                            sol, instance, self, endurance=drone_endurance)
                else:
                    # Basic: drone insertion only
                    sol, cost_saved, n_drones = apply_drone_postprocessing(
                        sol, instance, endurance=drone_endurance)
...
def run_pomo_improved(instance, n_runs=1, n_trucks=2, n_drones=0,
                       endurance=None, seed=42, variant='baseline',
                       tw_beta=0.5, model_path=None, check_tw_feasibility=True):
    """
    Interface compatible with week3 experiment runner.

    Args:
        variant: 'baseline' | 'tw_aware' | 'drone_only' | 'tw_aware_drone'
    ...
    if model_path is None:
        _W4_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'week4')
        model_path = os.path.join(_W4_DIR, 'algorithms', 'pomo', 'checkpoints', 'best_model.pt')
        if not os.path.exists(model_path):
            alt = os.path.join(_W4_DIR, 'algorithms', 'pomo', 'checkpoints', 'final_model.pt')
    ...
    end_name = endurance if isinstance(endurance, str) else 'medium'
...
# ── Import week4's spatial clustering for baseline ─────────────────────

import importlib.util
_w4_pomo_mt_path = os.path.join(_W4, 'pomo_multi_truck.py')
_w4_pomo_spec = importlib.util.spec_from_file_location("week4_pomo_mt", _w4_pomo_mt_path)
week4_pomo_mt = importlib.util.module_from_spec(_w4_pomo_spec)
sys.modules['week4_pomo_mt'] = week4_pomo_mt
_w4_pomo_spec.loader.exec_module(week4_pomo_mt)
