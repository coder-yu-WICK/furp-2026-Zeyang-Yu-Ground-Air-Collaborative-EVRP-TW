...
are uniformly non-binding. Model A = Model B = Model C, ΔCost=0%, E-Vio=0,
...
Differentiation mechanism:
  ...
  - Non-linear: 1.5× at 0-20% SOC, 1.0× at 20-80%, 0.5× at 80-100%
  - Charging from low SOC: non-linear is FASTER (1.5× rate) → lower charge_time
  - Charging from high SOC: non-linear is SLOWER (0.5× rate) → higher charge_time
  - Net effect: non-linear total charge_time ≠ linear → ΔCost(B-C) ≠ 0

Usage:
  python week7/run_ev_ablation.py --test           # Single instance quick test
  python week7/run_ev_ablation.py --quick           # 50c, 1 instance per type
  python week7/run_ev_ablation.py                   # Full study
"""
...
def run_model_a(instance, n_trucks, seed):
    ...
    if base_sol.tardiness > 1e-6:
        from repair import repair_tardiness_partial, repair_inter_route
    ...
    for route in routes_with_cs:
        ...
        for node_id in route:
            if prev > n_cust or node_id > n_cust:
                from ev_problem_model import station_distance
...
def print_ev_table(results):
    ...
    for r in results:
        ...
        ev_f_b = "✓" if b.get('ev_feasible', False) else "✗"
        ev_f_c = "✓" if c.get('ev_feasible', False) else "✗"
...
def print_charge_time_diff(results):
    ...
    print(f"  Non-linear: 1.5× rate at 0-20% SOC, 1.0× at 20-80%, 0.5× at 80-100%")
    print(f"{'=' * 120}")

    print(f"  {'Instance':<16s} {'Bat':>5s} {'Er':>4s} "
          f"{'ChgT(B)':>10s} {'ChgT(C)':>10s} {'ΔTime':>10s} {'Δ%':>8s} "
...
def main():
    ...
    for ik, n_trucks, tw_type in configs:
        ...
        for battery_kwh in battery_levels:
            for energy_rate in energy_rates:
                ...
                print(f"  Δ(C-B): cost={delta_cb:+.1f}  "
    ...
    if valid:
        ...
        print(f"     Avg ΔCost(B-A): {avg_delta_ba:+.1f} ({pct_ba:+.1f}% vs baseline)")
        ...
        print(f"\n  3. LINEAR vs NON-LINEAR DIFFERENTIATION:")
        print(f"     Configs with ΔCost(C-B) > 0.5: {n_differentiated}/{len(valid)}")
        if delta_cht_list:
            avg_delta_cht = sum(delta_cht_list) / len(delta_cht_list)
            print(f"     Avg ΔChargeTime(C-B): {avg_delta_cht:+.1f} min "
