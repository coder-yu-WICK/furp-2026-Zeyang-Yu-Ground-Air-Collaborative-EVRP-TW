...
from run_sota_expanded import run_one_method, METHOD_REGISTRY
...
est_sec = sum(nr * (2 if m[0].startswith('ours') or m[0] in ('sweep_nn','cw_savings','kmeans_nn','kmeans_2opt','sweep_pomo','cw_pomo')
                    else (40 if m[0]=='nsga2' else (200 if m[0]=='paco' else 5)))
              for m, nr in METHODS) * len(configs) / len(METHODS)
# Better estimate: per-instance time
