...
def kmeans_nearest_neighbor(instance, n_trucks, seed=42):
    ...
    # Import week4 spatial K-means
    import importlib.util
    _w4_path = os.path.join(_W4, 'pomo_multi_truck.py')
    _w4_spec = importlib.util.spec_from_file_location("_w4_pomo_mt_cluster", _w4_path)
    _w4_mod = importlib.util.module_from_spec(_w4_spec)
    _w4_spec.loader.exec_module(_w4_mod)

    clusters = _w4_mod.cluster_customers(instance, n_trucks)
...
def kmeans_two_opt(instance, n_trucks, seed=42, max_iter=100):
    ...
    import importlib.util
    _w4_path = os.path.join(_W4, 'pomo_multi_truck.py')
    _w4_spec = importlib.util.spec_from_file_location("_w4_pomo_mt_2opt", _w4_path)
    _w4_mod = importlib.util.module_from_spec(_w4_spec)
    _w4_spec.loader.exec_module(_w4_mod)
...
def sweep_pomo(instance, n_trucks, model_path=None, seed=42):
    ...
    if model_path is None:
        model_path = os.path.join(_W4, 'algorithms', 'pomo', 'checkpoints', 'best_model.pt')
        if not os.path.exists(model_path):
            model_path = os.path.join(_W4, 'algorithms', 'pomo', 'checkpoints', 'final_model.pt')
