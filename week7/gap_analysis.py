...
from config import (TRUCK_SPEED, TRUCK_CAPACITY, DEPOT,
                     TRUCK_DIST_COST_RATE, TRUCK_FIXED_COST)
from utils.data_loader import load_instance_from_disk, build_instance
...
def make_small_instance(source_inst, n_cust):
    ...
    from utils.data_loader import compute_distance_matrix
...
def enumerate_multi_route(customers, dist_matrix, depot, capacity, n_trucks):
    ...
    # For efficiency, generate all subsets for route 0, rest go to route 1
    for r in range(n + 1):
        for subset_combo in combinations(all_ids, r):
            ...
            if route0_ids:
                ...
                r0_remapped = []
            if route1_ids:
                # r1 uses temp IDs 1..len(route1_ids), need to map back
                r1_remapped = []
                for temp_id in r1:
