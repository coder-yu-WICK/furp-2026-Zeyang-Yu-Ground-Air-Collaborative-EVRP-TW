# -*- coding: utf-8 -*-
"""
NSGA-II: Non-dominated Sorting Genetic Algorithm II for Truck-Drone Routing.
Based on Deb et al. (2002), adapted for truck-drone collaborative routing.

Key features:
  - Non-dominated sorting for Pareto-based selection
  - Crowding distance for diversity preservation
  - SBX crossover + polynomial mutation
  - Chromosome encodes both truck routes and drone missions
"""
...
class NSGA2Solver:
    """NSGA-II solver for truck-drone collaborative routing."""
    ...
    def _random_chromosome(self):
        ...
        Format: [permutation_of_customers] + [drone_flags]
        drone_flags: for each customer, whether attempted by drone (1) or truck (0).
        """
        perm = list(range(1, self.n + 1))
        random.shuffle(perm)
        drone_flags = [1 if random.random() < 0.3 else 0 for _ in range(self.n)]
        return perm + drone_flags

    def _decode(self, chrom):
        """Decode chromosome to (truck_routes, drone_missions)."""
        n = self.n
        perm = chrom[:n]
        drone_flags = chrom[n:]

        # Determine which customers to attempt by drone
        drone_candidates = [perm[i] for i in range(n) if drone_flags[i] == 1]
        truck_customers = [perm[i] for i in range(n) if drone_flags[i] == 0]

        drone_missions = []
        drone_served = set()

        # Assign drone missions greedily
        for j in drone_candidates:
            if j in drone_served:
                continue
            cj = self.customers[j - 1]
            if cj['demand'] > DRONE_CAPACITY:
                truck_customers.append(j)
                continue

            # Find best launch and recovery nodes
            best_mission = None
            best_cost = float('inf')

            # Try using depot as launch/recovery
            d_depot_j = self._dist_depot(j)
            if d_depot_j * 2 <= self.endurance:
                best_mission = (0, j, 0)
                best_cost = d_depot_j * 2

            # Try inserting between truck customers
            for idx in range(len(truck_customers) - 1):
                i_node = truck_customers[idx]
                k_node = truck_customers[idx + 1]
                d_ij = self.dist[i_node][j]
                d_jk = self.dist[j][k_node]
                if d_ij + d_jk <= self.endurance:
                    cost = d_ij + d_jk
                    if cost < best_cost:
                        best_cost = cost
                        best_mission = (i_node, j, k_node)

            if best_mission:
                drone_missions.append(best_mission)
                drone_served.add(j)
            else:
                truck_customers.append(j)
    ...
    def _sbx_crossover(self, p1, p2):
        ...
        # Repair: ensure valid permutation for first n genes
        perm1 = self._repair_permutation(c1[:n])
        perm2 = self._repair_permutation(c2[:n])

        # Clip drone flags to [0, 1]
        flags1 = [1 if f > 0.5 else 0 for f in c1[n:]]
        flags2 = [1 if f > 0.5 else 0 for f in c2[n:]]

        return perm1 + flags1, perm2 + flags2
    ...
    def _mutate(self, chrom):
        """Polynomial mutation + swap mutation for permutation."""
        n = self.n
        mut = chrom[:]

        # Swap mutation for permutation part
        if random.random() < self.mut_pb:
            i, j = random.sample(range(n), 2)
            mut[i], mut[j] = mut[j], mut[i]

        # Bit flip for drone flags
        for i in range(n, len(mut)):
            if random.random() < self.mut_pb:
                mut[i] = 1 - mut[i]
    ...
    def solve(self):
        ...
        for gen in range(self.max_gen):
            ...
            solutions = [TruckDroneSolution(r, dm, self.instance)
                        for r, dm in decoded]
        ...
        final_solutions = [TruckDroneSolution(r, dm, self.instance)
                          for r, dm in final_decoded]
...
def run_nsga2(instance, n_trucks=2, n_drones=2, endurance=4.0, n_runs=10, seed=42):
    ...
    for run in range(n_runs):
        s = seed + run
        t0 = time.time()
        solver = NSGA2Solver(instance, n_trucks=n_trucks, n_drones=n_drones,
