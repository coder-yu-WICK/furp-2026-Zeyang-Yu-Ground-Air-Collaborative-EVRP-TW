...
Usage:
  python src/experiments/run_sync_study.py --test          # Single instance quick test
  python src/experiments/run_sync_study.py --quick          # 50c only, 1 rep
  python src/experiments/run_sync_study.py                  # Full study (50c/100c/200c)
  python src/experiments/run_sync_study.py --instance RC101_100c
"""
...
def print_comparison_table(results_by_instance):
    ...
    for r in results_by_instance:
        ...
        feas_c = "YES" if c['feasible'] else "NO"
        feas_d = "YES" if d['feasible'] else "NO"
...
def print_aggregate_summary(results_by_instance):
    ...
    for (tw_type, nc), results in sorted(groups.items()):
        ...
        print(f"    Delta:        cost={avg_d_cost - avg_c_cost:+.1f}  "
...
def main():
    ...
    for idx, cfg in enumerate(configs):
        ...
        print(f"  Delta: cost={delta_cost:+.1f}  drones={delta_drones:+d}  "
              f"sync_wait={result_d['sync_wait_time']:.1f}")
