...
from src.core.problem_model import (
...
from src.experiments.clustering_baselines import (
...
from src.pipeline.drone import apply_drone_dual

# ── Week 3 algorithm imports ─────────────────────────────────────────────
from src.algorithms.nsga2 import run_nsga2
from src.algorithms.paco import run_paco
from src.algorithms.ivnd import run_ivnd
...
def _dispatch_method(inst, cfg, method_key, method_info, seed):
    ...
    # ── Clustering-First Baselines ────────────────────────────────────
        ...
        from src.experiments.clustering_baselines import sweep_pomo_nn
...
def run_ours(instance, n_trucks, seed, use_repair=True,
    ...
    if n_drones_per_truck > 0:
        ...
        if final_sol.tardiness > 1e-6 and len(final_sol.drone_missions) > 0:
            from src.pipeline.repair import repair_tardiness_partial
        ...
        if final_sol.tardiness > 1e-6 and len(final_sol.drone_missions) == 0:
            from src.pipeline.repair import repair_inter_route
...
Examples:
  ...
  python week7/run_sota_expanded.py --instance RC101_50c
    ...
    # ── Clear src/algorithms cache ──
    _src = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _pycache = os.path.join(_src, 'algorithms', '__pycache__')
