...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'results', 'failure_cases')
...
def case1_battery_starvation():
    ...
    for label, battery_kwh, use_cs in [
        ...
        status = 'YES FEASIBLE' if sim['feasible'] else 'NO INFEASIBLE'
        print(f"\n  {label}: {status}")
    ...
    print(f"\n  >> ROOT CAUSE:")
    print(f"    Route energy demand (~{EV_ENERGY_RATE} kWh/km x route_length)")
...
def case2_time_window_tightening():
    ...
    for scale, use_repair in [
        ...
        try:
            ...
            status = 'YES' if sol.tardiness < 1.0 else f'NO tard={sol.tardiness:.0f}'
...
def case3_fleet_capacity():
    ...
    for label, r in [('Feasible route', route), ('Overloaded route', overloaded_route)]:
        ...
        status = 'YES FEASIBLE' if sol.feasible else 'NO INFEASIBLE'
...
def case4_drone_sync():
    ...
    print(f"\n  {'Mission':<20s} {'Truck(s)':>8s} {'Drone(s)':>8s} "
          f"{'D(s)':>8s} {'Hover?':>8s} {'Endurance':>10s}")
    print(f"  {'-'*65}")
...
def case5_charging_necessity():
    ...
    for n_cust_in_route in [10, 20, 30, 40, 50]:
        ...
        s1 = 'YES' if sim_no_cs['feasible'] else 'NO'
        s2 = 'YES' if sim_cs['feasible'] else 'NO'
