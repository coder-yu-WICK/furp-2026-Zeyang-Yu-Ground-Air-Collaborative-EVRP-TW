...
12 configs: 6 Solomon types x 200c x 14 methods + EV ablation
"""
...
from src.experiments.run_sota_expanded import (
...
def main():
    ...
    print(f'  {"=" * 90}')
    print(f'  Configs: {len(configs)} (6 Solomon types x 200c)')
    print(f'  Trucks: {N_TRUCKS}')
    print(f'  Standard methods: {len(METHODS)} x {len(configs)} instances = {total_standard} runs')
    print(f'  EV ablation: {len(EV_MODELS)} models x {len(configs)} instances = {total_ev} runs')
    ...
    for ci, cfg in enumerate(configs):
        ...
        if 'ours_full' in methods and 'ev_linear' in ev_methods:
            ...
            print(f'  {"-" * 80}')
            print(f'  EV Impact (linear): Dcost=+{ev_cost - base_cost:.1f} '
    ...
    print(f'{"=" * 90}')
    print(f'  {"Type":<8s} {"Model":<14s} {"DCost%":>8s} {"EV-Feas%":>9s} '
