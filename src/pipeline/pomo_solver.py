...
from src.pipeline.clustering import cluster_customers_tw_aware
from src.pipeline.adaptive_clustering import cluster_with_params
from src.pipeline.drone import apply_drone_postprocessing, insert_cross_route_drones
from src.pipeline.drone_reopt import reoptimize_drone_routes
from src.pipeline.cluster_feasibility import ensure_temporal_feasibility
...
class ImprovedPOMOSolver:
    ...
    def solve(self, instance, n_trucks, variant='baseline', n_runs=1,
        ...
        for run in range(n_runs):
            ...
            if strategy == 'spatial':
                from src.pipeline.pomo_multitruck import cluster_customers as spatial_cluster
            ...
            if use_drones:
                if use_reopt:
                    ...
                    from src.pipeline.drone_reopt import apply_drone_postprocessing_with_reopt
                    ...
                    sol, cost_saved, n_drones, drone_counts = apply_drone_postprocessing(
...
def run_pomo_improved(instance, n_runs=1, n_trucks=2, n_drones=0,
    ...
    if model_path is None:
        ...
        model_path = os.path.join(_SRC_DIR, 'algorithms', 'pomo', 'checkpoints', 'best_model.pt')
        if not os.path.exists(model_path):
            alt = os.path.join(_SRC_DIR, 'algorithms', 'pomo', 'checkpoints', 'final_model.pt')
