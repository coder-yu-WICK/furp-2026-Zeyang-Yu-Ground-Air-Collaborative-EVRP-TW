...
Usage:
  python src/experiments/analyze_results.py [results_json_path]
"""
...
from week8.experiments.statistical_tests import (
...
def main():
    ...
    print("\n-- 3b: Friedman Test -- Cost --")
    friedman_cost = friedman_test(cost_matrix)
    print_friedman_results(friedman_cost, method_keys)

    # ── 3c: Friedman Test (Tardiness) ──
    print("\n-- 3c: Friedman Test -- Tardiness --")
    friedman_tard = friedman_test(tard_matrix)
    print_friedman_results(friedman_tard, method_keys)

    # ── 3d: Nemenyi Post-Hoc ──
    if friedman_cost['significant_05']:
        print("\n-- 3d: Nemenyi Post-Hoc -- Cost --")
        nemenyi_cost = friedman_nemenyi_posthoc(cost_matrix)
        print_nemenyi_results(nemenyi_cost, method_keys)

    if friedman_tard['significant_05']:
        print("\n-- 3e: Nemenyi Post-Hoc -- Tardiness --")
        nemenyi_tard = friedman_nemenyi_posthoc(tard_matrix)
        print_nemenyi_results(nemenyi_tard, method_keys)
    ...
    if 'ours_full' in method_keys:
        ...
        for mkey in method_keys:
            ...
            sig_t = '*' if w_tard['significant_05'] else '  '
            sig_c = '*' if w_cost['significant_05'] else '  '
    ...
    if friedman_tard['significant_05']:
        our_rank_idx = method_keys.index('ours_full')
        our_rank = friedman_tard['method_ranks'][our_rank_idx]
        best_rank = min(friedman_tard['method_ranks'])
        print(f"\n  3. STATISTICAL SIGNIFICANCE (Tardiness):")
        print(f"     Friedman test: chi2={friedman_tard['statistic']:.1f}, "
              f"p={friedman_tard['p_value']:.4f} -- methods differ significantly")
        print(f"     Ours (Full) average rank: {our_rank:.2f}")
        if abs(our_rank - best_rank) < 0.01:
